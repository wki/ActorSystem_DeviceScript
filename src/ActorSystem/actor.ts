...
export abstract class Actor {
    ...
    private ensureMessageProcessing() {
        if (this.messages.length > 0 && this.processor == undefined && this.state === ActorState.Idle) {
            this.processor = setImmediate(this.processMessage.bind(this));
        }
    }
